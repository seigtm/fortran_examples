...
contains
   pure subroutine get_list_by_registration(surnames, initials, genders, registrations, avg_marks, &
      ...
      integer(I_)          :: registration_count, j
      integer, parameter   :: indexes(*) = [(j, j = 1, students_count)]
      ...
      allocate(surnames_registration(surname_length, registration_count), &
         initials_registration(initials_length, registration_count), &
         genders_registration(registration_count), &
         avg_marks_registration(registration_count))

      do concurrent(j = 1:registration_count)
         surnames_registration(:, j) = surnames(:, position_registration(j))
         initials_registration(:, j) = initials(:, position_registration(j))
         genders_registration(j)     =  genders(position_registration(j))
         avg_marks_registration(j)   = avg_marks(j)
      end do
