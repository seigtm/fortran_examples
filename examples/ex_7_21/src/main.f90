...
contains
   ...
   pure subroutine MaxNegAndMinPos(C, max_neg, min_pos, Mask, Indexes, Ind_max_neg, Ind_min_pos)
      ...
      !  Что в терминах задачи записывается на строке ниже?
      Ind_min_pos(:, 1) = Pack(Indexes(:, 1), Mask)
      !  ^ На этой строке в терминах задачи мы записали индексы
      !     минимальных положительных элементов по координате Y (строка).
      !      > [1 1 4].
      Ind_min_pos(:, 2) = Pack(Indexes(:, 2), Mask)
      !  ^ А на этой: координаты X (столбец).
      !     > [1 1 4 1 2 2].
      ! В результате:
      !  [1.10] [1.10]  2.20  3.30
      !   4.70   3.10   4.40  5.50
      !  -6.20   3.20  -6.20  5.50
      !   2.20  [1.10] -7.20  3.20
      ! Наименьшие из положительных:  1.10
      !   1  1 -> Matrix(строка=1, столбец=1)
      !   1  2 -> Matrix(строка=1, столбец=2)
      !   4  2 -> Matrix(строка=4, столбец=2)
