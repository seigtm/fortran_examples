...
contains
   ...
   pure recursive subroutine Get_list_by_gender(Stud, List, Amount, Gender)
	 ...
	 List => Null()  ! Зачем мы делаем это?
         ! Предположим, у нас такой порядок:
         !  "МДДДМММД", где нас интересуют М - мальчики.
         ! Вот то на то и выходит, что последнюю "Д"-евочку мы спокойно выкинем.
         ! Да, у нас есть условие "если пол == интересующий_нас", то:
         ! ...
         ! List%next => Null()
         ! ...
         ! Однако, если у нас был такой порядок: "ДДДДД", то мы нигде список так и не обнулим и вернём бред вместо Null()...
         ! То есть по сути мы делаем это на самом деле для одного сценария, в котором, например, у нас все студенты - "Д"-евочки,
         !  а мы ищем "М"-альчиков.
      end if
